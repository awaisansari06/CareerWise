...
export function getGeminiModel(modelName = "gemini-3.5-flash-lite") {
  const client = getGeminiClient();
  return client.getGenerativeModel({ model: modelName });
}

/**
 * Executes a Gemini generateContent call with a bounded timeout (25s) and bounded exponential backoff
 ...
 */
export async function safeGenerateContent(model, payload, { maxRetries = 2, timeoutMs = 25000 } = {}) {
  let lastError = null;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      const isTransient = status === 503 || errorMessage.includes("503") || errorMessage.toLowerCase().includes("unavailable");
